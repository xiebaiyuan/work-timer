...
function syncFromServer() {
  const url = 'https://url.com:1234/query?device_name=%E8%83%96';
...
function updateDisplay() {
  ...
  // 确定要显示的文本
  const displayText = currentTime === null ? 
    '今日未签到' : 
    `今天已工作：${currentTime}`;
...
function startAutoRefresh() {
  ...
  // 立即同步一次数据
  syncFromServer().then(() => {
    // 设置1秒更新一次显示的计时器（本地计算，无网络请求）
    displayIntervalId = setInterval(updateDisplay, 1000);
    
    // 设置60秒同步一次服务器数据的计时器
    syncIntervalId = setInterval(syncFromServer, 60000);
